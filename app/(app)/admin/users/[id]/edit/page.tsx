...
export default function EditUserPage() {
  ...
  useEffect(() => {
    ...
    if (!isSelfEdit && !canEditUser && !loading.user) {
      toast.error("Access Denied", {
        description: "You don't have permission to edit other users"
      });
      router.push("/admin/users");
    }
  }, [canViewUsers, canEditUser, isSelfEdit, loading.user, router]);
  ...
  const fetchUser = async () => {
    try {
      ...
      const data = await fetchWithAuth(`/api/users/${userId}?include_permissions=true`);
      if (data.success) {
        setUser(data.user);
        setFormData({
  ...
  const fetchActivities = async () => {
    try {
      ...
      const data = await fetchWithAuth(`/api/users/${userId}/activity?limit=10`);
      if (data.success) {
                ...
                <div className="space-y-4">
                  {sessions.map((session) => (
                    <div key={session.id} className="flex items-start gap-3 p-4 border rounded-lg">
                      <div className={`h-8 w-8 rounded-full ${session.is_active ? 'bg-green-100' : 'bg-gray-100'} flex items-center justify-center flex-shrink-0`}>
                        {session.is_active ? (
