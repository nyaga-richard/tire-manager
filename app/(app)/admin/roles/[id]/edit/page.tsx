...
export default function EditRolePage() {
  ...
  useEffect(() => {
    ...
    if (!canEditRole && !loading.role) {
      toast.error("Access Denied", {
        description: "You don't have permission to edit roles"
      });
      router.push("/admin/roles");
    }
  }, [canViewRoles, canEditRole, loading.role, router]);

  // Fetch role data
  useEffect(() => {
    if (token && roleId && canViewRoles) {
      fetchRole();
      fetchAllPermissions();
      fetchUsers();
    }
  }, [token, roleId, canViewRoles]);
  ...
  const fetchUsers = async () => {
    try {
      ...
      const data = await fetchWithAuth(`/api/roles/${roleId}/users?page=${userPagination.page}&limit=${userPagination.limit}`);
      if (data.success) {
        setUsers(data.users);
                    ...
                    <div className="space-y-1">
                      ...
                      <div>
                        <Badge 
                          ...
                          className={isSystemRole ? "bg-purple-100 text-purple-800 hover:bg-purple-100" : ""}
                        >
                          {isSystemRole ? "System Role" : "Custom Role"}
                        </Badge>
                      </div>
